...
import { getDynamicStringSegments, isDynamicSegment } from "./utils";
...
export async function cssFormatter(text: string) {
  ...
  return prettier
    .format(text, { parser: 'css', plugins: [parserPlugin], semi: false })
    .trim();
}
...
export async function htmlFormatter(text: string) {
  ...
  return prettier
    .format(text, { parser: 'html', plugins: [parserPlugin], semi: false })
    .trim();
}
...
async function getJavascriptFormatter() {
  ...
  return (text: string) =>
    prettier
      .format(text, { parser: 'babel', plugins: [parserBabel], semi: false })
      .trim();
}
...
export async function getJsonFormatter() {
  ...
  const parserBabel = await import('prettier/parser-babel');
  return (text: string) =>
    prettier.format(text, { parser: 'json', plugins: [parserBabel] }).trim();
}
...
async function getJsSegmentFormatter() {
  ...
  return (segment: string) =>
    '{{' + formatJsSegment(formatter, segment.slice(2, -2)) + '}}';
}
