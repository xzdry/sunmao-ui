      ...
      return (
        <Tr key={i}>
          <Td
            ...
            fontWeight="bold"
            onClick={() =>
              editorStore.setSelectedComponentId(result.position.componentId)
            }
          >
            {result.position.componentId}
          </Td>
          <Td fontWeight="bold">{result.position.traitType}</Td>
  ...
  return (
    ...
    >
      ...
      <Table
        size="sm"
        width="full"
        display={isCollapsed ? 'none' : 'table'}
        overflow="auto"
      >
        <Thead>
          <Tr>
        ...
        <Tbody maxHeight="150px">{errorItems}</Tbody>
      </Table>
    </VStack>
  );
});
